...
class Trainer(object):
    def __init__(self, rank, cfg, enable_amp=True):
        ...
        self.model2 = build_vos_model(cfg.MODEL_VOS, cfg).cuda(self.gpu)
        del self.model2.encoder
        self.engine = build_engine(
            cfg.MODEL_ENGINE,
            'train',
            aot_model=self.model,
            cycle_model=self.model2,
            gpu_id=self.gpu,
        ...
        if self.rank == 0:
            try:
                ...
                self.ema_params = get_param_buffer_for_ema(
                    self.model2, update_buffer=(not cfg.MODEL_FREEZE_BN))
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TRAIN_RESUME:
            ...
            try:
                ...
                self.model2, self.optimizer, removed_dict = load_network_and_optimizer(
                    self.model2,
                    self.optimizer,
                    resume_ckpt,
                    self.gpu,
            ...
            if cfg.PRETRAIN_FULL:
                self.model, removed_dict = load_network(
                    self.model, cfg.PRETRAIN_MODEL, self.gpu)
                self.model2, _ = load_network(
                    self.model2, cfg.PRETRAIN_MODEL, self.gpu)
                if len(removed_dict) > 0:
