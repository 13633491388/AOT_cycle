...
class Evaluator(object):
    def __init__(self, cfg, rank=0, seq_queue=None, info_queue=None):
        ...
        self.ensemble = build_vos_model(cfg.MODEL_VOS, cfg).cuda(self.gpu)
        self.gc=cfg.GC
        del self.ensemble.encoder
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TEST_CKPT_PATH is None:
            ...
            self.model, removed_dict = load_network(self.model,
                                                    cfg.TEST_CKPT_PATH,
                                                    self.gpu)
            if cfg.TEST_ENSEMBLE is not None:
                self.ensemble, _ = load_network(self.ensemble,
                                                cfg.TEST_ENSEMBLE,
                                                self.gpu)
            else:
                self.ensemble, _ = load_network(self.ensemble,
    ...
    def evaluating(self):
        ...
        with torch.no_grad():
            for seq_idx, seq_dataset in enumerate(self.dataset):
                ...
                seq_timers = []
                gc_engine=build_engine(cfg.MODEL_ENGINE,
                                             phase='gc',
                ...
                for frame_idx, samples in enumerate(seq_dataloader):
                    ...
                    for aug_idx in range(len(samples)):
                        ...
                        if frame_idx == 0:
                            ...
                            engine2.add_reference_frame(current_img,
                                                       _current_label,
                                                       frame_step=0,
                                                       obj_nums=obj_nums,
                                                       cycle=True
                                                       )
                            gc_engine.set_first(current_img,_current_label)
                        else:
                            if aug_idx == 0:
                                seq_timers.append([])
                            ...
                            engine2.match_propogate_one_frame(current_img,cycle=True)
                            pred_logit = engine2.decode_current_logits(
                                (ori_height, ori_width),cycle=True)

                            if is_flipped:
                                pred_logit = flip_tensor(pred_logit, 3)
                            
                            cycle_prob = torch.softmax(pred_logit, dim=1)
                            #all_preds.append(pred_prob)
                    if frame_idx > 0:
                        ...
                        if new_obj_label is not None:
                            ...
                            if not cfg.MODEL_USE_PREV_PROB:
                                ...
                                for aug_idx in range(len(samples)):
                                    engine = all_engines[aug_idx]
                                    current_prob =cycle_prob if frame_idx<10 else pred_prob
                                    ...
                                    engine2.update_memory(current_prob,cycle=True)
                                    current_label = flip_pred_label if samples[
                                        aug_idx]['meta']['flip'] else pred_label
