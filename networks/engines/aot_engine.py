...
class AOTgcEngine(nn.Module):
    ...
    def encode_one_img_mask(self, img=None, mask=None):
    ...
    def set_first(self,img,mask):
        first_embs, first_one_hot_mask = self.encode_one_img_mask(img, mask)
        self.curr_enc_embs=first_embs
    ...
    def add_reference_frame(self,
        ...
        curr_enc_embs, curr_one_hot_mask = self.encode_one_img_mask(img, mask)
        ...
        curr_lstt_output = self.AOT.LSTT_forward(curr_enc_embs,
                                                      None,
                                                      None,
                                                      curr_id_emb,
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        self.curr_long_term_memories=new_long_term_memories
    def gc(self,step=60000):
        self.curr_lstt_output = self.AOT.LSTT_forward(self.curr_enc_embs,
        ...
        curr_loss, curr_mask, curr_prob = self.generate_loss_mask(
                self.first_mask, step, return_prob=True)
...
class AOTEngine(nn.Module):
    def __init__(self,
        ...
        for k,v in self.AOT.named_parameters():
    ...
    def forward(self,
                ...
                use_prev_prob=False,
                cycle=False):  # only used for training
        ...
        if enable_prev_frame:
            ...
            self.match_propogate_one_frame(self.cycle_model)
            curr_loss, curr_mask, curr_prob = self.generate_loss_mask(
                self.offline_masks[self.frame_step], step,cycle=True, return_prob=True)
            self.update_short_term_memory(self.cycle_model,
                curr_mask,None if not use_prev_prob else curr_prob,
                None if use_prev_pred else self.assign_identity(self.cycle_model,
        ...
        self.match_propogate_one_frame(self.cycle_model)
        curr_loss, curr_mask, curr_prob = self.generate_loss_mask(
            self.offline_masks[self.frame_step], step, return_prob=True,cycle=True)
        ...
        index0=(self.offline_one_hot_masks[0].sum(3).sum(2)>0).sum(1)
        if cycle:
          
            for i in range(0,4):
                indexi=(self.offline_one_hot_masks[i+1].sum(3).sum(2)>0).sum(1)
                index=(indexi==index0)
                if index.sum():
                    onehotmask=one_hot_mask(curr_masks[i], self.max_obj_num)
                    self.add_reference_frame(self.AOT,mask=(onehotmask-curr_probs[i]).detach()+curr_probs[i],img_embs=self.offline_enc_embs[i])
                    self.reverse()
                    curr_closs, curr_cmask, curr_cprob = self.generate_loss_mask(
                    self.offline_masks[0], step, index=index,return_prob=True)
                    curr_closses.append(curr_closs)
            pred_closs = torch.cat(curr_closses, dim=0).mean(dim=0) if len(curr_closses)>0 else 0
            loss += pred_closs   
    ...
    def assign_identity(self, AOT,one_hot_mask):
        
        if self.enable_id_shuffle:
            one_hot_mask = torch.einsum('bohw,bot->bthw', one_hot_mask,
                                        self.id_shuffle_matrix)
        
        id_emb = AOT.get_id_emb(one_hot_mask).view(
            self.batch_size, -1, self.enc_hw).permute(2, 0, 1)
    ...
    def add_reference_frame(self,
        ...
        if self.pos_emb is None:
            self.pos_emb = AOT.get_pos_emb(curr_enc_embs[-1]).expand(
                self.batch_size, -1, -1,
                -1).view(self.batch_size, -1, self.enc_hw).permute(2, 0, 1)

        curr_id_emb = self.assign_identity(AOT,curr_one_hot_mask)
        self.curr_id_embs = curr_id_emb

        # self matching and propagation
        self.curr_lstt_output = AOT.LSTT_forward(curr_enc_embs,
                                                      None,
                                                      None,
                                                      curr_id_emb,
                                                      pos_emb=self.pos_emb,
    ...
    def set_prev_frame(self,AOT, img=None, mask=None, frame_step=1):
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        self.long_term_memories = updated_long_term_memories
        self.curr_long_term_memories=new_long_term_memories
    def update_short_term_memory(self, AOT,curr_mask, curr_prod=None,curr_id_emb=None):
        ...
        if curr_id_emb is None:
            ...
            if curr_prod is not None:
                curr_one_hot_mask= (curr_one_hot_mask-curr_prod).detach() +curr_prod
            curr_id_emb = self.assign_identity(AOT,curr_one_hot_mask)
        ...
        for layer_idx in range(len(lstt_curr_memories)):
            ...
            curr_v = AOT.LSTT.layers[layer_idx].linear_V(curr_v +
                                                              curr_id_emb)
    ...
    def match_propogate_one_frame(self,AOT, img=None, img_embs=None):
        self.frame_step += 1
        if img_embs is None:
        ...
        self.curr_lstt_output = AOT.LSTT_forward(curr_enc_embs,
                                                      ...
                                                      pos_emb=self.pos_emb,
                                                      size_2d=self.enc_size_2d)
    def reverse(self, img=None, img_embs=None):
        self.frame_step =0
        if img_embs is None:
            curr_enc_embs, _ = self.encode_one_img_mask(
                img, None, self.frame_step)
        ...
        self.curr_lstt_output = self.AOT.LSTT_forward(curr_enc_embs,
                                                      ...
                                                      pos_emb=self.pos_emb,
                                                      size_2d=self.enc_size_2d)
    def decode_current_logits(self,cycle, output_size=None):
        curr_enc_embs = self.curr_enc_embs
        curr_lstt_embs = self.curr_lstt_output[0]
         
        pred_id_logits = self.AOT.decode_id_logits(curr_lstt_embs,
                                                   curr_enc_embs) if cycle else self.cycle_model.decode_id_logits(curr_lstt_embs,curr_enc_embs)
    ...
    def calculate_current_loss(self, gt_mask, step,index=None):
        pred_id_logits = self.pred_id_logits
        ...
        for batch_idx, obj_num in enumerate(self.obj_nums):
          if index is None or index[batch_idx]:
            now_label = gt_mask[batch_idx].long()
            now_logit = pred_id_logits[batch_idx, :(obj_num + 1)].unsqueeze(0)
            label_list.append(now_label.long())
            logit_list.append(now_logit)
    ...
    def generate_loss_mask(self, gt_mask, step,cycle=False, index=None,return_prob=False):
        self.decode_current_logits(cycle)
        loss = self.calculate_current_loss(gt_mask, step,index)
        if return_prob:
            mask, prob = self.predict_current_mask(return_prob=True)
            return loss, mask, prob
        else:
...
class AOTInferEngine(nn.Module):
    ...
    def add_reference_frame(self, img, mask, obj_nums, frame_step=-1,cycle=False):
        if isinstance(obj_nums, list):
            obj_nums = obj_nums[0]
        aot_num = max(np.ceil(obj_nums / self.max_aot_obj_num), 1)
        while (aot_num > len(self.aot_engines)):
            new_engine = AOTEngine(self.AOT,self.cycle_model, self.gpu_id,
        ...
        for aot_engine, separated_mask in zip(self.aot_engines,
                                              separated_masks):
            model=aot_engine.AOT if not cycle else aot_engine.cycle_model
            aot_engine.add_reference_frame(model,img,
                                           separated_mask,
    ...
    def match_propogate_one_frame(self, img=None,cycle=False):
        
        img_embs = None
        for aot_engine in self.aot_engines:
            model=aot_engine.AOT if not cycle else aot_engine.cycle_model
            aot_engine.match_propogate_one_frame(model,img,img_embs=img_embs)
            if img_embs is None:  # reuse image embeddings
                img_embs = aot_engine.curr_enc_embs

    def decode_current_logits(self, output_size=None,cycle=False):
        all_logits = []
        for aot_engine in self.aot_engines:
            model=aot_engine.AOT if not cycle else aot_engine.cycle_model
            all_logits.append(aot_engine.decode_current_logits(model,output_size))
        pred_id_logits = self.soft_logit_aggregation(all_logits)
        return pred_id_logits

    def update_memory(self, curr_mask,cycle=False):
        separated_masks = self.separate_mask(curr_mask)
        
        for aot_engine, separated_mask in zip(self.aot_engines,
                                              separated_masks):
            model=aot_engine.AOT if not cycle else aot_engine.cycle_model
            aot_engine.update_short_term_memory(model,separated_mask)
